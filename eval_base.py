...
import pandas as pd
import statistics
from test import test_base_by_date, max_drawdown, save_res, get_test_setting


if __name__ == '__main__':
    # for dataset in ['acl18', 'sz_50']:
    for dataset in ['sz_50']:
        for model in ['LSTM_clf']:
        ...
        # for model in ['LSTM_clf', 'GRU_clf', 'MLP_clf', 'ALSTM_clf']:
            ...
            for seed in range(10):
                ...
                if dataset == 'acl18':
                    _, ret_lst, _ = test_base_by_date(model, net, test_date, dataset, stock_lst, feature_lst, target,
                                                      10, 4, 'clf', criterion)
                elif dataset == 'sz_50':
                    _, ret_lst, _ = test_base_by_date(model, net, test_date, dataset, stock_lst, feature_lst, target,
                                                      25, 4, 'clf', criterion)
